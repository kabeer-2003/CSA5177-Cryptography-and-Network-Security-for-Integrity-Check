...
unsigned *Algorithms_Hash_MD5(const char *msg, int mlen) {
    ...
    free(msg2); // Don't forget to free allocated memory
    return h;
}
...
char *GetMD5String(const char *msg, int mlen) {
    ...
    for (j = 0; j < 4; j++) {
        u.w = d[j];
        sprintf(str + j * 8, "%02x%02x%02x%02x", u.b[0], u.b[1], u.b[2], u.b[3]);
    }
    return str;
}

int main() {
    const char *msg = "Hello, MD5!";
    int mlen = strlen(msg);
    char *result = GetMD5String(msg, mlen);
    if (result != NULL) {
        printf("MD5: %s\n", result);
        free(result); 
    } else {
